...
for i in `seq 1 ${#sessnames[@]}`; do
	...
	mbname=`cat $tmp | grep "^\s*${restcount}\s\+.\+BOLD_${scanname}" | awk '{print $2}'`

	if [ -z "${mbname}" ]; then
		echo "No REST$i scan found for $subj"
		continue
		...
		#$apname
		#"
			echo $cmd $subj ${sessname} REST ${scanname} ${sbnum} ${restnum} ${panum} ${apnum}
		fi
	else
		cat $tmp
		#echo 2 ${mbname}
