...
StudyFolder="/home/range1-raid1/kjamison/Data/yuval" #Location of Subject folders (named by subjectID)
...
for Subject in $Subject_all; do
	...
	UnwarpAxis=y
	   ...
	   StartWithIdx=$idxInit
		...
		GradientDistortionCoeffs="/home/range1-raid1/kjamison/hcp_pipeline/grad_coeffs/7TAS_coeff_SC72CD_full.grad"
	...
	######################################################################################
	############### Make Fake Anatomical and anatomy-related xfms ########################
	NativeSpaceFolder="T1w"
	T1wFolder="${StudyFolder}"/"${Subject}"/"${NativeSpaceFolder}"

	mkdir -p ${subjAtlasSpaceFolder}/xfms
	mkdir -p ${T1wFolder}/xfms

	ScoutFile=${sbfile}
	fslmaths ${ScoutFile} ${subjAtlasSpaceFolder}/T1w_restore
	fslmaths ${ScoutFile} -mul 0 -add 1 ${subjAtlasSpaceFolder}/BiasField
	fslmaths ${ScoutFile} -mul 0 -add 1 ${subjAtlasSpaceFolder}/brainmask_fs
	convertwarp --relout -r ${ScoutFile} -m $FSLDIR/etc/flirtsch/ident.mat ${subjAtlasSpaceFolder}/xmfs/acpc_dc2standard

	#restore = biascorrected
	T1wImage="T1w_acpc_dc"
	T1wRestoreImage="T1w_acpc_dc_restore"
	T1wRestoreImageBrain="T1w_acpc_dc_restore_brain"

	imcp ${ScoutFile} ${T1wFolder}/${T1wImage}
	imcp ${ScoutFile} ${T1wFolder}/${T1wRestoreImage}
	fslmaths ${ScoutFile} -mul 0 -add 1 ${T1wFolder}/${T1wRestoreImageBrain}
