...
for Subject in $Subject_all ; do
  ...
  while [ $i -le $numT1ws ] ; do
    t1img="${StudyFolder}/${Subject}/unprocessed/Structural/T1w${i}.nii.gz"
    T1wInputImages="${T1wInputImages}${t1img}@"
    i=$(($i+1))
  done
  
  #pipeline won't run without T2w, so just copy T1 and pretend
  T2wInputImages=${T1wInputImages}
